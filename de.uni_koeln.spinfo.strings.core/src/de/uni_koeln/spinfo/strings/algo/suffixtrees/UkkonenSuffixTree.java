...
//    private List<Long> sequences;
    ...
    protected void addSingleSequence(List<Long> seq, int number) {
        ...
        for (; i < sequences.size(); i++) {
            ...
            for (; j <= i; j++) {
                ...
                // suffixLink
                while (currentNode != root && accessor.getSuffixLink(currentNode) == null
                        && canLinkJump)
    ...
    public Node jumpTo(Node starting, int from,
        ...
        while (canGoDown) {
            ...
            arrivedAt = (Node) accessor.getChildren(currentNode).get(
                    sequences.get(from));
            if (arrivedAt == null) {
                canGoDown = false;
                arrivedAt = currentNode;
...
//                        + to - from - 1;
    ...
    private void doRule2(Node parent, int splittingPos, int suffixStart,
        ...
        accessor.addChild(parent, sequences.get(splittingPos), leaf);
