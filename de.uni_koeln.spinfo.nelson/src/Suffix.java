...
//	 reach the final node.
	...
	{
	    if ( this.explicit() != 1 ) {
	        Edge edge = Application.ukkonen.find( origin_node, Application.t[ first_char_index ] );
	        int edge_span = edge.last_char_index - edge.first_char_index;
	        while ( edge_span <= ( last_char_index - first_char_index ) ) {
	            first_char_index = first_char_index + edge_span + 1;
	            origin_node = edge.end_node;
	            if ( first_char_index <= last_char_index ) {
	               edge = Application.ukkonen.find( edge.end_node, Application.t[ first_char_index ] );
	...
	public static void addPrefix( Suffix active, int last_char_index )
	{
	    ...
	    while(true){
	        ...
	        if ( active.explicit() == 1 ) {
	            edge = Application.ukkonen.find( active.origin_node, Application.t[ last_char_index ] );
	            if ( edge.start_node != -1 )
	                break;
	        } else { //implicit node, a little more complicated
	            edge = Application.ukkonen.find( active.origin_node, Application.t[ active.first_char_index ] );
	            int span = active.last_char_index - active.first_char_index;
	            if ( Application.t[ edge.first_char_index + span + 1 ] == Application.t[ last_char_index ] )
	                break;
	        ...
	        Edge new_edge = new Edge( last_char_index, Application.n, parent_node );
	        new_edge.insert();
