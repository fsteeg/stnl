...
public class Suffixtree {
	...
	public Edge find( int node, int c )
	{
	    int i = hash( node, c );
	    for ( ; ; ) {
	        if ( this.edges.get(i).start_node == node )
	            if ( c == Application.t[ this.edges.get(i).first_char_index ] )
	                return this.edges.get(i);
	        if ( this.edges.get(i).start_node == -1 )
	            return this.edges.get(i);
	...
	public void dump_edges( int current_n )
	{
		...
		for ( int j = 0 ; j < Suffixtree.HASH_TABLE_SIZE ; j++ ) {
			...
			if ( s.start_node == -1 )
				continue;
			System.out.println(s.start_node + "  " + s.end_node + "  " 
					+ Suffixtree.nodes.get(s.end_node).suffix_node + "  " 
			...
			for ( int l = s.first_char_index ; l <= top; l++ )
				System.out.println(Application.t[ l ]);
		}
	}
...
//	        System.out.println("comparing: " << ( T + Application.n - currentString.length + 1 )
//	             << " to " << currentString << endl;
//	        if ( strcmp(T + Application.n - currentString.length + 1, currentString ) != 0 )
